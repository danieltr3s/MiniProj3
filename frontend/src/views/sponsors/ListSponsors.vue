                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <tr v-for="sponsor of sponsors" :key="sponsor._id">
                                ...
                                <td class="pt-4">{{sponsor.quota}}</td>
                                <td>
                                    <router-link :to="{name:'editSponsor', params:{sponsorId: sponsor._id}}" tag="button"
                                        class="btn btn-outline-success mr-2 mt-2">
                                        <i class="fas fa-edit"></i> EDITAR
                                    </router-link>
                                    <button @click="viewSponsor(sponsor._id)" type="button"
                                        class="btn btn-outline-success mr-2 mt-2">
                                        <i class="fas fa-search"></i> VER
                                    </button>
                                    <button @click="removeSponsor(sponsor._id)" type="button"
...
export default {
    ...
    methods: {
        ...
        generateTemplate(sponsor) {
            let animal = this.animals.find(a => { return a.name === sponsor.animal});
            let url = '';
            if (animal !== undefined)
                url = animal.links[0].url;

            let response = `<br>
            <div style='width: 100%; height: 100%; display: flex; justify-content: center; flex-direction: column;'>
                <img src='${url}' alt='${sponsor.animal}' style="width: 400px; align-self: center;"/>
                <br>
                <p><b>NOME: </b>${sponsor.name}</p>
                <p><b>ANIMAL: </b>${sponsor.animal}</p>
                <p><b>QUOTA: </b>${sponsor.quota}</p>
            </div>`;
            return response;
        },
        removeSponsor(id) {
            this.$confirm(
                ...
                () => {
                    this.$store.dispatch(`sponsor/${REMOVE_SPONSOR}`, id).then(
                        ...
                        err => {
                            this.$alert(`${err.message}`, "Não foi possível remover o Patrocinador.", "error");
                        }
                    );
                },
                () => {
